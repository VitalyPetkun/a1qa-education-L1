...
public class AlertsTest extends BaseTest{
    ...
    public void checkAlerts() {
        ...
        Assert.assertTrue(alertsWindowsPage.getAlertsForm().getAlertUtils().getTextAlert().contains(ConfigManager.getTestDataString("textAlertToSee")),
                "Alert with the text 'You clicked a button' is not open");

        alertsWindowsPage.getAlertsForm().getAlertUtils().clickBtnOk();
        Assert.assertTrue((!alertsWindowsPage.getAlertsForm().getAlertUtils().isAlertOpen()), "Alert did not close.");

        alertsWindowsPage.getAlertsForm().clickBtnConfirm();
        Assert.assertTrue(alertsWindowsPage.getAlertsForm().getAlertUtils().getTextAlert().contains(ConfigManager.getTestDataString("textAlertConfirm")),
        ...
        Assert.assertTrue(alertsWindowsPage.getAlertsForm().getAlertUtils().getTextAlert().contains(ConfigManager.getTestDataString("textAlertPromt")),
                "Alert with the text 'Please enter your name' is not open");

        alertsWindowsPage.getAlertsForm().getAlertUtils().inputText(ConfigManager.getTestDataString("inputTextAlert"));
        alertsWindowsPage.getAlertsForm().getAlertUtils().clickBtnOk();
        Assert.assertTrue(!alertsWindowsPage.getAlertsForm().getAlertUtils().isAlertOpen(), "Alert did not close.");
